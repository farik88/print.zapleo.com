...
class Orders extends \common\components\base\BaseActiveRecord
{
    ...
    public function attributeLabels()
    {
        return [
            'id' => 'ID',
            'delivery_id' => Yii::t('backend_orders','Доставка'),
            'payment_id' => Yii::t('backend_orders','Оплата'),
            'coupon_id' => Yii::t('backend_orders','Купон'),
            'user_id' => Yii::t('backend_orders','Пользователь'),
            'total' => Yii::t('backend_orders','Сумма'),
            'comment' => Yii::t('backend_orders','Коментарий'),
            'status_payment' => Yii::t('backend_orders','Статус оплаты'),
            'status_delivery' => Yii::t('backend_orders','Статус доставки'),
            'data' => Yii::t('backend_orders','Дата'),
            'address' => Yii::t('backend_orders','Адрес'),
        ];
    }
    ...
    {
        return $this->hasMany(\backend\models\OrderCart::className(), ['order_id' => 'id']);
    }
    ...
    {
        return $this->hasOne(\backend\models\Deliveries::className(), ['id' => 'delivery_id']);
    }
    ...
    {
        return $this->hasOne(\backend\models\Payments::className(), ['id' => 'payment_id']);
    }

...
class LangRequest extends Request{
    ...
    public function getLangUrl()
    {
        if ($this->_lang_url === null) {
	    	...
	    	if (isset($url_list[1]) && $url_list[1] == 'admin') {
                $lang_url = isset($url_list[2]) ? $url_list[2] : null;
            } else {
    ...
    protected function resolvePathInfo()
    {
        ...
        // TODO: Переделать
        if ($baseUrl == '/admin' && strpos($pathInfo, Lang::getCurrent()->url) === 0) {
            $pathInfo = substr($pathInfo, strlen(Lang::getCurrent()->url)+1);
        }
